...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body style="margin-bottom: 30px;">
        ...
        <div class="afisareIBAN" style="text-align: center; margin-top: 30px;">
            ...
            <p>Nu există date!</p>
            @else
            <form method="post" action="{{ route('actualizare') }}">
            @csrf
            @foreach($select as $sel)
                <p>Anul - {{$sel->anul}}</p>
                <input type="hidden" id="an" name="an" value="{{$sel->anul}}">

                @foreach($fulleco as $ec)
                    @if($ec->kd_eco == $sel->kd_eco)
                        <p>Eco - {{$sel->kd_eco}} - {{$ec->label_md}}</p>
                        <input type="hidden" id="eco" name="eco" value="{{$sel->kd_eco}}">
                    @endif
                @endforeach
                ...
                @foreach($localitati as $loc)
                    @if($loc->kd_local == $sel->kd_local)
                        ...
                        <input type="hidden" id="localitate" name="localitate" value="{{$sel->kd_local}}">
                    @endif
                @endforeach

                <p>{{$sel->iban}}</p>
            @endforeach
            <input type="button" class="btn btn-primary" id="modificaIBAN" onclick="modIBAN()" value="Modifică IBAN">
            <div class="form-group" id="ibNou" style="display: none;">
                <label for="iban">IBAN nou</label>
                ...
                <button type="submit" class="btn btn-primary">Actualizează IBAN</button>
            </div>
            </form>
            @endif
        </div>
